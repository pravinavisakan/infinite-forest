...
// a new function - returns a function that inserts a shape based on an obj file (to handle asynchronous fie load)
const insertObj = (filename, shape_transformation = Mat4.identity(), pre_transformation = Mat4.identity(), post_transformation = Mat4.identity()) => {
	return (model_transform, recipient) => {
		let transform = model_transform.times(pre_transformation)
		recipient.load_obj_file(filename, transform, shape_transformation);
		return transform.times(post_transformation);
	}
}
